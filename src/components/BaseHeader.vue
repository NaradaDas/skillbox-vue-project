<template lang="">
  <header class="header container">
    <div class="header__wrapper">
      ...
      <router-link
        :class="{ 'header__cart--loading': loading }"
        :to="{ name: 'basket' }"
        class="header__cart"
        href="cart.html"
        aria-label="Корзина с товарами"
      >
        <BaseSmallLoader class="header__cart--preloader" :sending="loading" />
        <svg
          width="24px"
          height="24px"
          viewBox="0 0 24 24"
        ...
        >
          <path
            ...
            2.25 12 2.25V3.75ZM7.24999 7V11H8.74999V7H7.24999ZM15.25 7V11H16.75V7H15.25Z"
            fill="#001A72"
          />
        </svg>
        <span
          :class="{ 'header__count--no-count': isOrderInfoPage }"
          class="header__count"
...
export default defineComponent({
  ...
  setup() {
    const { fetchBasket } = useApi();
    const store = useStore();
    const route = useRoute();
    const isOrderInfoPage = ref(false);
    watch(
      () => route.name,
      (value) => {
        isOrderInfoPage.value = value === 'orderInfo';
      },
    );

    onMounted(() => (!isOrderInfoPage.value ? fetchBasket() : 1));

    return {
      isOrderInfoPage,
      total: toRef(store.state, 'basketItemsQuantity'),
      loading: toRef(store.state, 'isBasketLoading'),
    };
  },
});
</script>
<style>
.header__count--no-count {
  display: none;
}
